...
export default class TaskView extends Component {
  constructor(props) {
    ...
    const { tasks } = props;
    const currentData = tasks.filter(
      item => item.id === props.currentTaskId
    )[0];
    const {
      name,
      title,
  ...
  resolveTicket = () => {
    ...
    const currentData = tasks.filter(
      item => item.id === this.props.currentTaskId
    )[0];
    const assigne = currentData.assigne;
    const assigneResolve = users.find(
      user => `${user.firstName} ${user.lastName}` === assigne
    );
    const estimateResolve = currentData.estimate.split(" ");
    const taskProgress = moment(currentData.startProgressAt)
      .fromNow()
      .split(" ");

    if (
      (estimateResolve[0] > taskProgress[0] &&
      ...
      UserActions.updateUser(assigneResolve.id, {
        resolvedTasks: assigneResolve.resolvedTasks + 1,
        completedNotOnTime: assigneResolve.completedNotOnTime + 1
      });
      TasksActions.updateTask(currentTaskId, { state: "Resolved" });
    } else {
      UserActions.updateUser(assigneResolve.id, {
        resolvedTasks: assigneResolve.resolvedTasks + 1,
        completedOnTime: assigneResolve.completedOnTime + 1
      });
      TasksActions.updateTask(currentTaskId, { state: "Resolved" });
    }
  };
  ...
  closeTicket = () => {
    ...
    UserActions.updateUser(assigneResolve.id, {
      closedTasks: [...assigneResolve.closedTasks, currentData.name]
    });
  ...
  reopenTicket = () => {
    ...
    UserActions.updateUser(assigneResolve.id, {
      reopenedTasks: assigneResolve.reopenedTasks + 1
    });
  ...
  removeTask = () => {
    const { closePanel, taskDelete, currentTaskId } = this.props;

    taskDelete(currentTaskId);
    closePanel();
  };

  render() {
    const { tasks, comments, currentTaskId, taskDelete } = this.props;
    const { editTask } = this.state;
    const currentData = tasks.filter(
      item => item.id === this.props.currentTaskId
    )[0];
    const dateString =
      currentData.startProgressAt &&
      moment(currentData.startProgressAt).fromNow();
    ...
    return (
      <div className="task-view">
        ...
        <div className="task-modal__buttons">
          {!editTask && (
            <Fragment>
              <button className="task-view__button" onClick={this.onEditToggle}>
                Edit Task
              </button>
              <button className="task-view__button">Go to PDF</button>
              <button className="task-view__button" onClick={this.removeTask}>
                Remove task
              </button>
            </Fragment>
          )}
          {editTask && (
            <button className="task-view__button" onClick={this.editTask}>
              Apply changes
            </button>
          )}
        </div>
        <div className="task-modal">
          ...
          {editTask ? (
            ...
            <div className="task-modal__overview">
              <span className="task-modal__header">Assigne</span>
              <span className="task-modal__text">{currentData.assigne}</span>
              <span className="task-modal__header">Estimate</span>
              <span className="task-modal__text">{currentData.estimate}</span>
              <span className="task-modal__header">Type of task</span>
              <span className="task-modal__text">{currentData.type}</span>
              <span className="task-modal__header">Priority</span>
              <span
                className="task-modal__text"
              ...
              <span className="task-modal__text">{currentData.state}</span>
              {(currentData.state === "In progress" ||
                ...
                <div className="task-modal__progress">
                  ...
                  <span className="task-modal__text">{dateString}</span>
                </div>
              )}
            </div>
          )}
