...
export default class TaskEditor extends Component {
  ...
  handleTaskAdd = () => {
    if (this.state.free) {
      const minTasks = Math.min.apply(null, this.props.users.map(user => user.assignTasks.length));
      const maxCompletedOnTime = Math.max.apply(null, this.props.users.map(user => user.completedOnTime));
      const minCompletedNotOntime = Math.min.apply(null, this.props.users.map(user => user.completedNotOnTime));
      const freeUser = this.props.users.find(user => user.assignTasks.length === minTasks && minCompletedNotOntime === user.completedNotOnTime && maxCompletedOnTime === user.completedOnTime);
      UserActions.updateUser(freeUser.id, { assignTasks: [...freeUser.assignTasks, this.state.name] });
    } else {
      const updateUser = this.props.users.find(user => user.firstName === this.state.user.split(' ')[0]);
      const assignTasks = updateUser.assignTasks;
      UserActions.updateUser(updateUser.id, { assignTasks: [...assignTasks, this.state.name] });
    }
    ...
    this.setState({ name: '', status: '', text: '', type: '', estimate: '', state: 'Planned', title: '', free: false, color: '#FFFFFF', user: '' });
  }
      ...
      <div className='task-editor'>
        ...
        <div className='task-editor__footer'>
          <div className='checkbox-field'>
            <span
              className='task-editor__checkbox-text'
            >
              Assign with efficiency
                    </span>
            <input disabled={this.state.user} className="task-editor__checkbox" type="checkbox" id="check" onClick={this.handleAssignToFree}></input>
          </div>
          <button
